...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [14 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,14);
	...
	copyCArrayToM_double((&(output.x1[0])), mxGetPr(outvar), 7);
	mxSetField(plhs[0], 0, "x1", outvar);


	/* column vector of length 7 */
	outvar = mxCreateDoubleMatrix(7, 1, mxREAL);
	copyCArrayToM_double((&(output.x2[0])), mxGetPr(outvar), 7);
