...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if( mxGetM(par) != 24 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [24 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,24);
	...
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [48 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,48);
	...
	copyCArrayToM_double((&(output.x1[0])), mxGetPr(outvar), 12);
	mxSetField(plhs[0], 0, "x1", outvar);


	/* column vector of length 12 */
	outvar = mxCreateDoubleMatrix(12, 1, mxREAL);
	copyCArrayToM_double((&(output.x2[0])), mxGetPr(outvar), 12);
