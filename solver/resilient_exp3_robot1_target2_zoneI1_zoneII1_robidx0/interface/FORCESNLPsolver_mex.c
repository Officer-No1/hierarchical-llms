...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
	...
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [36 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,36);
	...
	plhs[0] = mxCreateStructMatrix(1, 1, 2, outputnames);
		/* column vector of length 6 */
	outvar = mxCreateDoubleMatrix(6, 1, mxREAL);
	copyCArrayToM_double((&(output.x1[0])), mxGetPr(outvar), 6);
	mxSetField(plhs[0], 0, "x1", outvar);


	/* column vector of length 6 */
	outvar = mxCreateDoubleMatrix(6, 1, mxREAL);
	copyCArrayToM_double((&(output.x2[0])), mxGetPr(outvar), 6);
